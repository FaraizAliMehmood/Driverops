...
import FlightFeed, { FlightFeedHandle } from "@/components/FlightFeed";
import ZoneRecommendation from "@/components/ZoneRecommendation";
import EarningsTracker from "@/components/EarningsTracker";
import WeatherAlert, { WeatherAlertHandle } from "@/components/WeatherAlert";
import StatsOverview from "@/components/StatsOverview";

const Index = () => {
  const [lastUpdated, setLastUpdated] = useState<Date>(new Date());
  const [refreshKey, setRefreshKey] = useState<number>(0);
  const weatherRef = useRef<WeatherAlertHandle>(null);
  const flightFeedRef = useRef<FlightFeedHandle>(null);
    ...
    <div className="min-h-screen bg-gradient-hero">
      <div className="container mx-auto px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <WeatherAlert ref={weatherRef} />
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          <div className="lg:col-span-2">
            <FlightFeed ref={flightFeedRef} />
          </div>
          <div>
